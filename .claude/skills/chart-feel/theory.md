...
- **注意**: 音種の加点は「音種が散らばっている曲」でしか accent にならない。
  四つ打ちでキックが鳴り続ける曲(ユウダチはキック43%・スネア6%)では ↓ に
  引力がかかり続け、反対側のレーンが痩せる。均す項(まんべんなく)は、この引力を
  打ち消せる強さが要る。弱すぎると名前どおりの働きをしない。
...
- **このゲームでは**: 交互加点だけだと 2 レーンの往復が延々と続きうる。
  レーンを均す項が効いている遅い流れ(easy)では、「最も長く空いたレーン」が
  常に一周ぶん前と一致するため、←↑↓→ の 4 周回が機械的に続く。
- **落とし込み**: 生成。assign_lanes で同じ往復が続いたら減点し、別のレーンへ流す。
  遅い流れでは、直前3つと候補で4レーン揃う(=ちょうど一周する)ときも減点する。
- **測り方**: レーン列の 4 連パターンの種類数と最頻パターンの占有率。周回に
  なっていると種類が一桁・最頻が 25% 前後に張り付く(健全なら数十種類・数%)。
...
- 検査化済みの原則(1, 2, 5, 7)は bake-chart.py が自動で見張る(5 は生成側の絶対条件でもある)
