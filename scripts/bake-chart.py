...
SONGS: dict[str, Song] = {
    ...
    "yudachi": Song(
        id="yudachi",
        src=Path("public/assets/hosomiyudachidance.mp3"),
        # 既定の探索域(95-185)では 4/3 倍の 113.335 が1位で出るが、それだと
        # 16分グリッドの位相別「音がある率」が 0.92/0.71/0.71/0.89 とガタガタになる。
        # --bpm 70-135 で測り直した 84.988 だと 0.94/0.93/0.97/0.95 と揃い、
        # 小節頭の低域も 1 拍目が明確に立つ。キックの実間隔 0.353 秒(=8分)とも合う
        bpm=84.988,
        beat0=0.4520,
        bar0=0.4520,
        last_bar=43,     # 小節43(121.9秒)以降はアウトロのフェード
        song_end=126.0,  # 実尺 127.5 秒
        targets={"easy": 147, "normal": 257, "hard": 422},
        sections=[
            (0, 4, 0.50),    # イントロ。「あ、きた ゆうだち」だけで静か
            (4, 9, 0.80),    # Aメロ。軒下
            (9, 13, 0.95),   # プリサビ。ためて上がる
            (13, 17, 1.25),  # サビ1
            (17, 21, 1.20),  # ラップ前半。歌が刻むので手数を出す
            (21, 25, 1.05),  # ラップ後半。伴奏が抜ける
            (25, 26, 0.35),  # ブレイク。「せーの!」で音が止まる
            (26, 30, 1.15),  # 間奏。歌がないぶん手を動かす
            (30, 35, 1.30),  # ラスサビ
            (35, 39, 0.85),  # アウトロの歌
            (39, 43, 0.70),  # 伴奏だけのアウトロ
        ],
        # 「さん に いち」はカウントで方向ではないのでギミックは置かない
    ),
}
